function [rt_err, sim_log, all_dets_history, saved_rd_at_30, sys_out, nodes_out, targets_out, nodes_true_out] = ... 
    tx_sea_1(error_mode, error_mask, cfg)
% =========================================================================
% V22.1 PHANTOM SLAYER (Sea-Only) - 海面舰船目标版
% 流程（简化）：
%   1. 子系统内相干合成 -> 子系统RD图
%   2. CFAR检测 -> 候选点 (r_meas, v_meas)
%   3. 子系统内多链路OTDA定位 + 一致性确认 (≥50%链路支持)
%   4. 子系统间非相干融合 (位置关联) (仅sea子系统)
%   5. 将确认点迹转换为跟踪器输入 (直接使用粗定位结果)
%   6. 跟踪器 
%
% 输入参数：
%   error_mode : 0-无误差, 1-有误差无补偿, 2-有误差部分补偿, 3-有误差传统补偿
%   error_mask : [dT_en, dF_en, dPhi_en, dPos_en] 二值掩码
%   cfg        : 可选配置结构体，用于覆盖默认参数。字段包括：
%                - sys: 系统参数覆盖 (如 B, fc, Nsc, Nc, 节点构型等)
%                - targets: 目标参数覆盖 (如 rcs, pos, vel)
%                - errors: 误差参数覆盖 (如 dT, dF, dPhi, dPos)
%                - 若某字段未提供，则使用内部默认值。
% =========================================================================
...
% 合并配置结构体（深度合并）
cfg = merge_config_with_defaults(cfg);

clc; rng(7);
fprintf('========== V22.1 PHANTOM SLAYER (Sea-Only) ==========\n');
fprintf('误差模式: ');
if isempty(error_mode) && ~isempty(error_mask)
    fprintf('掩码模式\n');
else
    if error_mode == 0, fprintf('0 (无误差)\n');
    elseif error_mode == 1, fprintf('1 (有误差无补偿)\n');
    elseif error_mode == 2, fprintf('2 (有误差现有方法补偿)\n');
    elseif error_mode == 3, fprintf('3 (有误差传统方法补偿)\n');
    else, fprintf('%d\n', error_mode);
    end
end

%% ================== 1) 系统参数 ==================
sys = get_default_system_params();
% 应用cfg.sys覆盖
sys = apply_config_overrides(sys, cfg, 'sys');
...
%% ================== 2) 误差配置 ==================
sys.errors.enabled = false; sys.errors.dT = 2e-8; sys.errors.dF = 50;
sys.errors.dPhi = deg2rad(4); sys.errors.dPos = [15,10,10];
sys.errors.compensated = false; sys.errors.compensation_factor = 0.0;
...
if isfield(cfg, 'errors')
    flds = fieldnames(cfg.errors);
    for i = 1:length(flds)
        sys.errors.(flds{i}) = cfg.errors.(flds{i});
    end
end
...
targets = make_targets(cfg);   % <--- 函数已改为舰船目标
...
for t = 0:dt:T_sim
    t_loop = tic;
    sys.current_time = t;
    [nodes, targets] = update_physics(nodes, targets, t, dt);  % <--- 增加了舰船垂荡更新
    ...
    % ---------- 单次海面检测（不再区分强弱、MTI多模式、速度分组） ----------
    sys_det = sys;                % 使用标准MTI设置
    sys_det.use_mti = true;
    sys_det.mti_mode = 2;         % 使用双脉冲对消，适合低速舰船杂波抑制
    sys_det.remove_slowtime_mean = true;
    sys_det.accum_enable = true;
    
    [confirmed_sea, rd_show_sea, nWork_sea] = process_subsystem_enhanced(...
        nodes, targets, sys_det, clu, noise, cfar, txSym, codeMask, [], 'sea');
    
    % 融合（仅sea子系统，实际无融合必要，但保留结构）
    fused_dets = fuse_detections_subsystems(confirmed_sea, [], sys);
    solved_plots = convert_confirmed_to_solved(fused_dets, sys);
    
    total_dets = numel(fused_dets);
    all_dets_history{end+1} = struct('t', t, 'fused', fused_dets);
    
    nWork_total = nWork_sea;
    rd_show = rd_show_sea;
    ...
    if nargout == 0 && mod(t, 1.0) < 1e-9
        if isempty(rd_show)
            rd_show = zeros(sys.Nsc*sys.Ncodes, sys.Nc);
        end
    ...
    fprintf('t=%.2f  work=%3d  dets=%d sol=%d trk=%d | loop=%.3fs\n', ...
        t, nWork_total, total_dets, numel(solved_plots), numel(mgr.tracks), toc(t_loop));

    if abs(t - 30) < dt/2
        saved_rd_at_30 = struct('time', t, 'rd_sea', rd_show_sea, 'rd_air', []);
    end
end
...
%% ================== 新增：默认系统参数函数 ==================
function sys = get_default_system_params()
sys.c  = 3e8;
sys.fc = 500e6;
sys.B   = 2e6;
sys.Nsc = 1024;
...
%% ================== 新增：配置合并与覆盖 ==================
function cfg = merge_config_with_defaults(cfg)
% 若cfg为空，返回空结构
if isempty(fieldnames(cfg))
    return;
end
% 可在此处设置默认配置字段，但当前直接使用输入的cfg
end

function s = apply_config_overrides(s, cfg, fieldname)
% 将cfg.(fieldname)中的字段覆盖到结构体s的同名字段
if isfield(cfg, fieldname)
    override = cfg.(fieldname);
    flds = fieldnames(override);
    for i = 1:length(flds)
        s.(flds{i}) = override.(flds{i});
    end
end
end
...
for tx_id = tx_ids
    ...
    for rx_id = rx_ids
        ...
        for cid = cid_list
            ...
            % 修正参数顺序：gen_rx_ofdm_link_code_fixed(tx, rx, targets, sys, X, cid, clu, noise, tg_override)
...
% 若cfg中指定了节点配置，则覆盖
if nargin > 0 && isfield(cfg, 'nodes')
    nodes = cfg.nodes;
elseif nargin > 0 && isfield(cfg, 'node_count') && cfg.node_count == 1
    % 单节点场景（仅保留Sea1）
    nodes = nodes(1);
elseif nargin > 0 && isfield(cfg, 'node_geometry')
    % 自定义构型示例：可在此扩展
    geo = cfg.node_geometry;
    if strcmp(geo, 'baseline_short')
        nodes(1).pos = [0, 6000, 0];
        nodes(2).pos = [0, -6000, 0];
    elseif strcmp(geo, 'baseline_long')
        nodes(1).pos = [0, 20000, 0];
        nodes(2).pos = [0, -20000, 0];
    end
end
end

function targets = make_targets(cfg)
% 默认三艘舰船目标（带垂荡参数）
targets(1).id   = 1;
targets(1).name = 'Ship1-FAC';
targets(1).pos  = [60000, 3000, 0];
targets(1).vel  = [-11.0,  8.0, 0];
targets(1).vel0 = targets(1).vel;
targets(1).rcs  = 20.0;
targets(1).type = 'SEA';
targets(1).heaveAmp = 1.2;
targets(1).heaveHz = 0.18;
targets(1).heavePh = 0.3;

targets(2).id   = 2;
targets(2).name = 'Ship2-Frigate';
targets(2).pos  = [82000, -4000, 0];
targets(2).vel  = [-12.0,  10.0, 0];
targets(2).vel0 = targets(2).vel;
targets(2).rcs  = 30.0;
targets(2).type = 'SEA';
targets(2).heaveAmp = 1.5;
targets(2).heaveHz = 0.14;
targets(2).heavePh = 2.0;

targets(3).id   = 3;
targets(3).name = 'Ship3-Destroyer';
targets(3).pos  = [70000, 1000, 0];
targets(3).vel  = [-17.0,  13.0, 0];
targets(3).vel0 = targets(3).vel;
targets(3).rcs  = 35.0;
targets(3).type = 'SEA';
targets(3).heaveAmp = 1.8;
targets(3).heaveHz = 0.12;
targets(3).heavePh = 1.2;

% 若cfg中指定了目标参数，则覆盖
if nargin > 0 && isfield(cfg, 'targets')
    flds = fieldnames(cfg.targets);
    for i = 1:length(flds)
        % 假设cfg.targets是一个结构体数组，与targets尺寸匹配
        if isstruct(cfg.targets) && length(cfg.targets) == length(targets)
            for k = 1:length(targets)
                targets(k).(flds{i}) = cfg.targets(k).(flds{i});
            end
        else
            % 如果是标量值，应用到所有目标
            val = cfg.targets.(flds{i});
            for k = 1:length(targets)
                targets(k).(flds{i}) = val;
            end
        end
    end
end
...
for k=1:numel(targets)
    ...
    % 添加垂荡运动（若目标定义了垂荡参数）
    if isfield(targets(k),'heaveAmp') && isfield(targets(k),'heaveHz') && isfield(targets(k),'heavePh')
        heave = targets(k).heaveAmp * sin(2*pi*targets(k).heaveHz * t + targets(k).heavePh);
        targets(k).pos(3) = heave;   % 垂荡仅影响高度，不影响平面位置
    end
end
end
...
gh.fig = figure('Name','V22.1 PHANTOM SLAYER (Sea-Only)','Color','w','Position',[50,50,1700,850]);
...
s = sprintf('Time: %.2f s | V22.1 PHANTOM SLAYER (Sea-Only) | dt=0.5s\n', t);
...
for k = 1:N_targets
    ...
    init_pos_err = sys.init_sigma_pos * randn(3,1);
    init_vel_err = sys.init_sigma_vel * randn(3,1);
...
A = log(0.62 / Pfa);
B = log(0.62 / Pd);
factor = 6.2 + 4.54 ./ sqrt(Np + 0.44);
snr_db = -5 * log10(Np) + factor * log10(A + 0.12*A.*B + 1.7*B);
end
...
function plot_core_performance(sys, sim_log, rt_err, nodes, targets, all_dets_history, rdBank, saved_rd_at_30)
    ...
    SNR0_dB = 18; R0_km = 80; R0_m = R0_km*1e3;
    Pfa = 1e-6;
    RCS_list = [20, 30, 35];   % 舰船RCS
    target_names = {'Ship1','Ship2','Ship3'};
    colors = {'r','g','b'};
    ...
    N_coh_sea = 4;
    N_coh_air = 9;
    N_eq = N_coh_sea + N_coh_air;
    N_noncoh = 6;
    
    %% 图1：检测概率 vs 距离
    figure('Name','Core1: Detection Probability','Color','w');
    dist_km = 30:5:250;
    ...
    hold on; grid on;
    for k = 1:3
        Pd_th_sys = zeros(size(dist_km));
        Pd_th_non = zeros(size(dist_km));
        for j = 1:length(dist_km)
            R = dist_km(j)*1e3;
            SNR_single = 10^( (SNR0_dB + 10*log10(RCS_list(k)) - 40*log10(R/R0_m)) /10 );
            SNR_sys = SNR_single * N_eq;
            SNR_non = SNR_single * N_noncoh;
            Pd_th_sys(j) = 1 - ncx2cdf(chi2inv(1-Pfa,2), 2, 2*SNR_sys);
            Pd_th_non(j) = 1 - ncx2cdf(chi2inv(1-Pfa,2), 2, 2*SNR_non);
        end
        plot(dist_km, Pd_th_sys, '--', 'Color', colors{k}, 'LineWidth',1.5, ...
            'DisplayName', [target_names{k} ' Theory (2-level)']);
        plot(dist_km, Pd_th_non, ':', 'Color', colors{k}, 'LineWidth',1.5, ...
            'DisplayName', [target_names{k} ' Theory (6-link noncoh)']);
        if any(Pd_sim(k,:) > 0)
            plot(dist_km, Pd_sim(k,:), 'o-', 'Color', colors{k}, ...
                'DisplayName', [target_names{k} ' Sim'], 'MarkerSize',6);
        end
    end
    xlabel('Distance (km)'); ylabel('Detection Probability P_d');
    title('Detection Performance: 2-level Coherent/Noncoherent vs Full Noncoherent');
    legend('Location','southwest');
    ylim([0 1]);
    
    %% 图2：定位精度CRLB vs 实际RMSE
    figure('Name','Core2: Localization Accuracy','Color','w');
    hold on; grid on;
    for k = 1:3
        idx = find(id_all == k);
        if isempty(idx), continue; end
        ...
        crlb_th = zeros(size(dist_tgt));
        for i = 1:length(dist_tgt)
            R = dist_tgt(i)*1e3;
            SNR_single = 10^( (SNR0_dB + 10*log10(RCS_list(k)) - 40*log10(R/R0_m)) /10 );
            SNR_sys = SNR_single * N_eq;
            sigma_r = c/(2*B*sqrt(2*SNR_sys));
            crlb_th(i) = 2.5 * sigma_r;
        end
        plot(dist_tgt, err_interp, '.', 'Color', colors{k}, 'MarkerSize',4);
        plot(dist_tgt, crlb_th, '-', 'Color', colors{k}, 'LineWidth',1.5, ...
            'DisplayName', [target_names{k} ' CRLB']);
    end
    xlabel('Distance (km)'); ylabel('Position RMSE (m)');
    title('Localization Accuracy: CRLB vs Simulated');
    legend show;
    ylim([0 800]);
    
    %% 图3：跟踪精度随时间变化
    figure('Name','Core3: Tracking Accuracy','Color','w');
    hold on; grid on;
    if ~isempty(rt_err.time)
        plot(rt_err.time, rt_err.e1, 'r-', 'LineWidth',1.5);
        plot(rt_err.time, rt_err.e2, 'g-', 'LineWidth',1.5);
        plot(rt_err.time, rt_err.e3, 'b-', 'LineWidth',1.5);
    end
    ...
    %% 图4：最大探测距离
    figure('Name','Core4: Max Detection Range','Color','w');
    Pd_target = 0.9;
    R_max_sim = nan(1,3);
    for k = 1:3
        idx = find(id_all == k);
    ...
    R_max_non = zeros(1,3);
    for k = 1:3
        SNR_req_single = albersheim_snr_db_fixed(Pfa, Pd_target, 1);
        SNR_avail_single = SNR0_dB + 10*log10(RCS_list(k));
        R_max_sys(k) = R0_km * 10^((SNR_avail_single - (SNR_req_single - 10*log10(N_eq)))/40);
        R_max_non(k) = R0_km * 10^((SNR_avail_single - (SNR_req_single - 10*log10(N_noncoh)))/40);
    end
    bar_width = 0.25;
    x = 1:3;
    bar(x - bar_width, R_max_non, bar_width, 'FaceColor', [0.7 0.7 0.7], 'DisplayName', 'Theory (6-link noncoh)');
    hold on;
    bar(x, R_max_sys, bar_width, 'FaceColor', [0.2 0.6 0.8], 'DisplayName', 'Theory (2-level)');
    bar(x + bar_width, R_max_sim, bar_width, 'FaceColor', 'b', 'DisplayName', 'Simulation');
    set(gca, 'XTick', x, 'XTickLabel', target_names);
    ylabel('Max Range (km)');
    legend('Location','northwest');
    title('Maximum Detection Range (P_d=0.9)');
    grid on;
end